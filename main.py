...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(f"[TEXT] Usuário: {user.id} | Mensagem: {user_text}")

    try:
        logger.info("[FLOW] Enviando para orquestrador...")
        mensagem_texto, mensagem_audio = handle_message(user_text)

        logger.info("[FLOW] Gerando áudio (TTS)...")
        audio_file = speak(mensagem_audio)

        logger.info("[FLOW] Enviando resposta ao usuário...")
        await update.message.reply_text(mensagem_texto)
...
async def handle_audio(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.info("[FLOW] Enviando para orquestrador...")
        mensagem_texto, mensagem_audio = handle_message(texto)
